...
export const t = {
  ...
  templateFieldPlaceholder: (name: string) =>
    msg("popup_template_field_placeholder", [String(name)]),
  ...
  favoriteModalSaveChanges:
    msg("popup_favorite_modal_save_changes") || "Save changes",
  favoriteEditTitle: msg("popup_favorite_edit_title") || "Edit favorite",
  favoriteEditDesc:
    msg("popup_favorite_edit_desc") ||
    "Update targets, defaults, chain steps, and schedule settings.",
  ...
  favoriteTargetsLabel:
    msg("popup_favorite_targets_label") || "Default target services",
  favoriteTagsLabel: msg("popup_favorite_tags_label") || "Tags",
  favoriteFolderLabel: msg("popup_favorite_folder_label") || "Folder",
  favoritePinnedLabel:
    msg("popup_favorite_pinned_label") || "Pin this favorite",
  favoriteScheduleEnabledLabel:
    msg("popup_favorite_schedule_enabled_label") || "Enable scheduled run",
  favoriteScheduledAtLabel:
    msg("popup_favorite_scheduled_at_label") || "Next run time",
  favoriteScheduleRepeatLabel:
    msg("popup_favorite_schedule_repeat_label") || "Repeat",
  favoriteScheduleRepeatNone:
    msg("popup_favorite_schedule_repeat_none") || "One time",
  favoriteScheduleRepeatDaily:
    msg("popup_favorite_schedule_repeat_daily") || "Daily",
  favoriteScheduleRepeatWeekday:
    msg("popup_favorite_schedule_repeat_weekday") || "Weekdays",
  favoriteScheduleRepeatWeekly:
    msg("popup_favorite_schedule_repeat_weekly") || "Weekly",
  ...
  favoriteChainDesc:
    msg("popup_favorite_chain_desc") ||
    "Each step runs in order and stops the chain if any step fails.",
  favoriteChainAddStep: msg("popup_favorite_chain_add_step") || "Add step",
  favoriteStepLabel: (index: number) =>
    msg("popup_favorite_step_label", [String(index)]) || `Step ${index}`,
  ...
  favoriteStepDelayLabel:
    msg("popup_favorite_step_delay_label") ||
    "Delay after previous step (ms)",
  favoriteStepTargetsLabel:
    msg("popup_favorite_step_targets_label") || "Override targets",
  favoriteStepTargetsHint:
    msg("popup_favorite_step_targets_hint") ||
    "Leave empty to use the favorite default targets.",
  favoriteRunNow: msg("popup_favorite_run_now") || "Run now",
  favoriteRunQueued: msg("popup_favorite_run_queued") || "Favorite run queued.",
  favoriteRunNeedsEditor:
    msg("popup_favorite_run_needs_editor") ||
    "This favorite needs more input before it can run.",
  favoriteScheduleDateRequired:
    msg("popup_favorite_schedule_date_required") ||
    "Choose the next run time for this schedule.",
  favoriteChainNeedsStep:
    msg("popup_favorite_chain_needs_step") ||
    "Add at least one non-empty chain step.",
  favoriteKindSingle: msg("popup_favorite_kind_single") || "Single",
  favoriteKindChain: msg("popup_favorite_kind_chain") || "Chain",
  favoriteScheduledBadge:
    msg("popup_favorite_scheduled_badge") || "Scheduled",
  favoriteStepCount: (count: number) =>
    msg("popup_favorite_step_count", [String(count)]) || `${count} steps`,
  favoriteEdit: msg("popup_favorite_edit") || "Edit",
  clearPrompt: msg("popup_clear_prompt") || "Clear",
  promptCounter: (current: number) =>
    isKorean ? `${current}자` : `${current} chars`,
  serviceManagementTitle:
    msg("popup_service_management_title") || "Service Management",
  serviceManagementDesc:
    msg("popup_service_management_desc") ||
    "Add, edit, enable, or disable AI targets without editing code.",
  addService: msg("popup_service_add") || "Add Service",
  resetServices: msg("popup_service_reset") || "Reset Services",
  resetServicesConfirm:
    msg("popup_service_reset_confirm") ||
    "Reset built-in services and remove all custom services?",
  ...
  serviceEditorAddTitle:
    msg("popup_service_editor_add_title") || "Add Custom Service",
  serviceEditorEditTitle:
    msg("popup_service_editor_edit_title") || "Edit Service",
  serviceEditorDesc:
    msg("popup_service_editor_desc") ||
    "Configure selector, submit strategy, style, and launch URL.",
  serviceFieldName: msg("popup_service_field_name") || "Service Name",
  serviceFieldUrl: msg("popup_service_field_url") || "Service URL",
  serviceFieldInputSelector:
    msg("popup_service_field_input_selector") || "Input Selector",
  serviceFieldInputType:
    msg("popup_service_field_input_type") || "Input Type",
  serviceFieldSubmitSelector:
    msg("popup_service_field_submit_selector") || "Submit Selector",
  serviceFieldSubmitMethod:
    msg("popup_service_field_submit_method") || "Submit Method",
  serviceFieldAdvanced:
    msg("popup_service_field_advanced") || "Advanced Settings",
  serviceFieldFallbackSelectors:
    msg("popup_service_field_fallback_selectors") || "Fallback Selectors",
  serviceFieldAuthSelectors:
    msg("popup_service_field_auth_selectors") || "Auth Selectors",
  serviceFieldHostnameAliases:
    msg("popup_service_field_hostname_aliases") || "Hostname Aliases",
  serviceFieldSupportedRoutes:
    msg("popup_service_field_supported_routes") || "Supported Routes",
  serviceFieldVerifiedAt:
    msg("popup_service_field_verified_at") || "Verified Date",
  serviceFieldVerifiedRoute:
    msg("popup_service_field_verified_route") || "Verified Route",
  serviceFieldVerifiedAuthState:
    msg("popup_service_field_verified_auth_state") || "Verified Auth State",
  serviceFieldVerifiedLocale:
    msg("popup_service_field_verified_locale") || "Verified Locale",
  serviceFieldVerifiedVersion:
    msg("popup_service_field_verified_version") || "Verified Version",
  ...
  serviceResetDone:
    msg("popup_service_reset_done") || "Service settings were reset.",
  servicePermissionDenied:
    msg("popup_service_permission_denied") ||
    "Host permission was not granted for this service URL.",
  serviceValidationError:
    msg("popup_service_validation_error") ||
    "Please check the service form fields.",
  serviceTestNoTab:
    msg("popup_service_test_no_tab") ||
    "No active tab is available for selector testing.",
  serviceTestNoSelector:
    msg("popup_service_test_no_selector") || "Enter a selector first.",
  serviceTestInvalidTab:
    msg("popup_service_test_invalid_tab") ||
    "Selector testing only works on http/https tabs.",
  serviceTestSuccess: (inputType: string) =>
    msg("popup_service_test_success", [String(inputType)]) ||
    `Element found (${inputType})`,
  serviceTestFail: msg("popup_service_test_fail") || "Element not found.",
  serviceTestError: (message: string) =>
    msg("popup_service_test_error", [String(message)]) ||
    `Selector test failed: ${message}`,
  serviceEmptyList: msg("popup_service_empty_list") || "No services available.",
  selectorWarningTooltip:
    msg("popup_selector_warning_tooltip") ||
    "Selector may have changed. Review the service config.",
  restoredBroadcastSending:
    msg("popup_broadcast_restored_sending") ||
    "Previous broadcast is still running.",
  ...
  toastSendSuccess: (count: number) =>
    msg("toast_send_success", [String(count)]) || `${count} services queued.`,
  toastPromptEmpty:
    msg("toast_prompt_empty") ||
    msg("popup_warn_empty") ||
    "Please enter a prompt.",
  toastNoService:
    msg("toast_no_service") || "Please select at least one service.",
  toastSelectorFailed: (name: string) =>
    msg("toast_selector_failed", [String(name)]) ||
    `${name} selector was not found.`,
  historySortLatest: msg("popup_history_sort_latest") || "Latest",
  historySortOldest: msg("popup_history_sort_oldest") || "Oldest",
  historySortMostSuccess:
    msg("popup_history_sort_most_success") || "Most success",
  historySortMostFailure:
    msg("popup_history_sort_most_failure") || "Most failure",
  favoriteSortRecentUsed:
    msg("popup_favorite_sort_recent_used") || "Recent use",
  favoriteSortUsageCount:
    msg("popup_favorite_sort_usage_count") || "Usage count",
  favoriteSortTitle: msg("popup_favorite_sort_title") || "Title",
  favoriteSortCreatedAt:
    msg("popup_favorite_sort_created_at") || "Created date",
  waitMultiplierLabel: msg("popup_wait_multiplier_label") || "Wait multiplier",
  waitMultiplierValue: (value: number) =>
    msg("popup_wait_multiplier_value", [String(Number(value).toFixed(1))]) ||
    `${Number(value).toFixed(1)}x`,
  resendModalTitle: msg("popup_resend_modal_title") || "Resend History Item",
  resendModalDesc:
    msg("popup_resend_modal_desc") ||
    "Choose which services to resend this prompt to.",
  ...
  importReportDesc:
    msg("popup_import_report_desc") ||
    "Review accepted, rewritten, and rejected items from this import.",
  importReportClose: msg("popup_import_report_close") || "Close",
  importReportVersion: msg("popup_import_report_version") || "Version",
  importReportAccepted:
    msg("popup_import_report_accepted") || "Accepted services",
  importReportRewritten:
    msg("popup_import_report_rewritten") || "Rewritten IDs",
  importReportBuiltins:
    msg("popup_import_report_builtins") || "Built-in adjustments",
  importReportRejected:
    msg("popup_import_report_rejected") || "Rejected services",
  importReportRejectedEmpty:
    msg("popup_import_report_rejected_empty") || "No rejected services.",
  importRejectReason: (reason: string) =>
    msg(`popup_import_reject_${reason}`) || reason,
  ariaSelected: msg("popup_aria_selected") || "selected",
  ariaNotSelected: msg("popup_aria_not_selected") || "not selected",
  reuseTabsLabel:
    msg("popup_reuse_tabs_label") ||
    "Reuse open AI tabs in the current window by default",
  reuseTabsDescEnabled:
    msg("popup_reuse_tabs_desc_enabled") ||
    "When no tab is chosen explicitly, the broadcaster reuses a matching open AI tab before opening a new one.",
  ...
  openTabsUseDefaultDetail: (modeLabel: string) =>
    msg("popup_open_tabs_use_default_detail", [String(modeLabel)]) ||
  ...
  openTabsAlwaysNewDetail:
    msg("popup_open_tabs_always_new_detail") ||
    "Ignore matching open tabs for this service only.",
