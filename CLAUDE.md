...
- **프로젝트**: `prompt-broadcaster`
- **Spec Kit 초기화**: `.specify/ 있음`
- **에이전트 스킬**: Grok=True, Claude=True, Codex/Agy(.agents)=True
- **활성 기능 디렉터리**: `specs/001-prompt-broadcast-v1-1` (포인터: `.specify/feature.json`)
- **기능 제목**: 멀티 AI 프롬프트 동시 전송 · 셀렉터 안정화
- **산출물**: spec=`yes`, plan=`True`, research/data-model/quickstart=`True`, tasks=`False`, converge=`False`

### 에이전트가 먼저 읽을 파일

1. `specs/001-prompt-broadcast-v1-1/spec.md` — 무엇을/왜 (사용자 스토리, FR, 성공 기준)
2. `specs/001-prompt-broadcast-v1-1/plan.md` — 기술 컨텍스트·구조 결정
3. `specs/001-prompt-broadcast-v1-1/tasks.md` — 실행 가능 작업 목록 (`[x]`=이미 있음, `[ ]`=잔여)
4. `specs/001-prompt-broadcast-v1-1/research.md`, `data-model.md`, `quickstart.md`, `contracts/` — 설계 보조
...
```text
# 현재 기능 파악
read specs/001-prompt-broadcast-v1-1/spec.md
read specs/001-prompt-broadcast-v1-1/tasks.md
# 잔여 구현
